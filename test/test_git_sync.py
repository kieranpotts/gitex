"""
Test suite for git-sync command.
"""
...
@pytest.fixture
def test_script():
    """Get the path to the git-sync script"""

    repo_dir = Path(__file__).parent.parent
    script_path = repo_dir / "bin" / "git-sync"

    assert script_path.exists(), f"git-sync script not found at {script_path}"

    return str(script_path)


class TestGitSync:
    """Test cases for git-sync command"""
