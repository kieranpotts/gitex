...
@pytest.fixture
def test_script():
    """Get the path to the git-graph script"""

    repo_dir = Path(__file__).parent.parent
    script_path = repo_dir / "bin" / "git-graph"

    assert script_path.exists(), f"git-graph script not found at {script_path}"

    return str(script_path)


class TestGitGraph:
    """Test cases for git-graph command"""
