"""
Test suite for git-state command.
"""
...
@pytest.fixture
def test_script():
    """Get the path to the git-state script"""

    repo_dir = Path(__file__).parent.parent
    script_path = repo_dir / "bin" / "git-state"

    assert script_path.exists(), f"git-state script not found at {script_path}"

    return str(script_path)


class TestGitState:
    """Test cases for git-state command"""
