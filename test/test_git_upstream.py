...
@pytest.fixture
def test_script():
    """Get the path to the git-upstream script"""

    repo_dir = Path(__file__).parent.parent
    script_path = repo_dir / "bin" / "git-upstream"

    assert script_path.exists(), f"git-upstream script not found at {script_path}"

    return str(script_path)


class TestGitUpstream:
    """Test cases for git-upstream command"""
